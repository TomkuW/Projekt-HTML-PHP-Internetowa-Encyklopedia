...
if($mysqli -> query("DELETE FROM kompedium WHERE kompedium_id ='$id' LIMIT 1")){
						...
						echo 'window.location.href = "strona_glowna.php";';
						echo '</script>';
}
}
...
<div class="main">
	...
	<form class="form-inline" action="encyklopedia.php">
    <div class="input-group">
      <input type="word" class="form-control" size="50" placeholder="Podana fraza" required>
      <div class="input-group-btn">
        <button type="button" class="btn btn-danger">Szukaj</button>
      </div>
    </div>
  </form>
	</div>	
	...
	<hr>
      <h4 >Ostatnio dodane:</h4>
	  	...
	  	<?php 
					...
					$resulti = $mysqli->query("SELECT p.kompedium_id , k.nazwa, p.fraza, p.tresc, p.data_dodania, u.imie, u.nazwisko FROM kompedium p, uzytkownik u, kategoria k WHERE u.uzytkownik_id = p.uzytkownik_id AND p.kategoria_id = k.kategoria_id ORDER BY kompedium_id DESC LIMIT 4");
					while ( $article = mysqli_fetch_array($resulti) ) {
						echo '<article>';
						...
						echo '</br>';
						echo '<a href="strona_glowna.php?id=' . $article['kompedium_id'] . '">';
						echo '<div class="btn btn-warning btn-xs" ><i>Usuń</i></div>';
						echo '</a>';
						echo '<hr>';
						echo '</article>';
